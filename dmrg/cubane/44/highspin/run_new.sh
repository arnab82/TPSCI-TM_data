...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /projects/nmayhall_lab/arnab/dmrg/bimetallics/cubane/44/highspin/FCIDUMP ./FCIDUMP
        cp /projects/nmayhall_lab/arnab/dmrg/bimetallics/cubane/44/highspin/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "0" = "1" ]; then
    cp /projects/nmayhall_lab/arnab/dmrg/bimetallics/cubane/44/highspin/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi
