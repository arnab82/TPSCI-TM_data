...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /projects/nmayhall_lab/arnab/dmrg/bimetallics/Mn_IV_O2_dimer/lowspin/FCIDUMP ./FCIDUMP
        cp /projects/nmayhall_lab/arnab/dmrg/bimetallics/Mn_IV_O2_dimer/lowspin/${SCPT}.conf ${SCPT}.conf.${TJ}
...
if [ "0" = "1" ]; then
    cp /projects/nmayhall_lab/arnab/dmrg/bimetallics/Mn_IV_O2_dimer/lowspinnode0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi
