...
cc = ['#FF0000', '#0000FF', '#FF00FF',  '#800000', '#008000', '#000080', '#808000', '#800080', '#008080', '#808080', '#C0C0C0', '#FF0000', '#00FF00', '#0000FF', '#FFFF00', '#FF00FF', '#00FFFF', '#800000', '#008000', '#000080', '#808000', '#800080', '#008080', '#808080', '#C0C0C0']
fig, ax = plt.subplots()
...
for key in energy_var:
    ...
    for j in range(len(x)):
        ax.plot(x[j], y[j], marker='o', linestyle='-', markersize=8, color=cb[key+1])
        ax.plot(x[j], z[j], marker='x', linestyle=' ', markersize=8, color=cb[key+1])
    
    x2 = np.array([-1,0])*conversion
    line = m*x2+b
    print("y axis values are", line)
    print("b value is ", b)
    ax.plot(x2, line, alpha=1.0, color=cb[key+1], linestyle='-', linewidth=1.5, label="S=%d" % key )
    line = m2*x2+b 
    print("y axis values are", line)                                  
    ax.plot(x2, line, alpha=0.5, color = cb[key+1], linestyle='--', linewidth=1.5,label='S=%d'%key)
...
fig.savefig(file+'_extrap_wavenumber.pdf', dpi=300, bbox_inches='tight')
